<template>
  <div class="max-w-md mx-auto px-4 py-6">
    ...
    <h1 class="text-xl font-bold mb-4">🏃 ประวัติการออกกำลังกาย</h1>
    ...
    <div v-else class="space-y-3">
      <div v-for="(rec, i) in records" :key="rec.id" class="bg-green-50 rounded-lg shadow p-4">
        ...
        <div class="font-bold text-green-700 text-sm">{{ statusLabel(rec.status) }}</div>
...
const LABELS: Record<string, string> = {
  ...
  regular: "ออกกำลังกายสม่ำเสมอ",
  sometimes: "ออกกำลังกายบางครั้ง",
  none: "ไม่ได้ออกกำลังกาย",
};
...
onMounted(async () => {
  try {
    const res = await $fetch<{ success: boolean; data?: Record<string, string>[] }>(
      `/api/exercise-record?pid=${encodeURIComponent(nationalId)}`
    );
