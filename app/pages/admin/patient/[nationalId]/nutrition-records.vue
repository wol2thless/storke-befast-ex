<template>
  <div class="max-w-md mx-auto px-4 py-6">
    ...
    <h1 class="text-xl font-bold mb-4">🥗 ประวัติการรับประทานอาหาร</h1>
    ...
    <div v-else class="space-y-3">
      <div v-for="(rec, i) in records" :key="rec.id" class="bg-orange-50 rounded-lg shadow p-4">
...
const LABELS: Record<string, string> = {
  normal_full: "อาหารครบ 5 หมู่",
  normal_notfull: "อาหารไม่ครบ 5 หมู่",
  less: "ทานได้น้อย",
  cannot: "ไม่สามารถทานได้",
};
...
function statusColor(s?: string) {
  if (s === "normal_full") return "text-green-600";
  if (s === "normal_notfull") return "text-yellow-600";
  if (s === "less" || s === "cannot") return "text-red-600";
...
onMounted(async () => {
  try {
    const res = await $fetch<{ success: boolean; data?: Record<string, string>[] }>(
      `/api/nutrition-record?pid=${encodeURIComponent(nationalId)}`
    );
